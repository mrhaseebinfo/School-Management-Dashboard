...
const schema = z.object({
  username: z
    .string()
    .min(3, { message: 'Username must be at least 3 characterws long!' })
    .max(20, { message: 'Username must be at most 20 characterws long!' }),
  ...
  birthday: z.string().min(1, { message: 'Birthdayis required!' }),
...
const StudentForm = ({ type, data }: { type: "create" | "update"; data?: any }) => {
  ...
  return (
    <form className="flex flex-col gap-8" onSubmit={onSubmit}>
      ...
      <span className='text-xs text-gray-400 font-medium'>Authentication Informaton</span>
      {/* Input Field */}
      ...
      <span className='text-xs text-gray-400 font-medium'>Personal Informaton</span>
      <div className='flex justify-between flex-wrap gap-4'>
        ...
        <InputField label='Blood' name='bloodType' defaultValue={data?.bloodType} register={register} error={errors?.bloodType} />
        ...
        <div className='flex flex-col gap-2 w-full md:w-1/4'>
          ...
          {errors.sex?.message && (
            <p className='text-xs text-white p-2 rounded-md'>{errors.sex.message.toString()}</p>
          )}
        </div>
        ...
        <div className='flex flex-col gap-2 w-full md:w-1/4 justify-center'>
          ...
          {errors.img?.message && (
            <p className='text-xs text-white p-2 rounded-md'>{errors.img.message.toString()}</p>
          )}
        </div>
      </div>
